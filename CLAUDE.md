...
- **Legacy spawn 硬閘（`ALLOW_LEGACY_SPAWN`）**: 本機 spawn 派工（PTY/SDK）預設**停用**——`AgentManager.startAgent` 與 `ExecutionPipeline.executeTask` 未設 `ALLOW_LEGACY_SPAWN=1` 一律 throw。受影響面：Web UI 的 agent.add／Spec mode 執行、Asana auto-execute、Axure crawler（mockup.crawl）、SkillGenerator。執行一律走外部 Claude Code session + MCP；MCP 合成 agent（`mcp-{taskId}`）與啟動恢復（kill-only）不受影響。
