...
export function ProjectSettings() {
  ...
  // Load global SVN credentials
  useEffect(() => {
    if (!client || svnCredsLoaded) return;
    const unsub = client.addMessageListener((msg) => {
      if (msg.type === 'config.state') {
        const p = msg.payload as { svnUsername: string; hasSvnPassword: boolean };
        setSvnUsername(p.svnUsername || '');
        // Don't overwrite password field if user has typed something
        if (!svnCredsLoaded) {
          setSvnPassword(p.hasSvnPassword ? '••••••••' : '');
        }
        setSvnCredsLoaded(true);
      }
    });
    client.send({
      type: 'config.get',
  ...
  const handleSave = useCallback(() => {
    ...
    // Save SVN credentials globally (only if password was actually changed, not the placeholder)
      ...
      <div className="space-y-6">
        ...
        <div className="border border-border rounded-lg p-4 space-y-3">
          ...
          <div className="border-t border-border/50 pt-3 mt-3">
            <p className="text-[10px] text-muted-foreground/70 mb-2">SVN 帳號密碼（全域設定，所有專案共用）</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-muted-foreground mb-1">SVN Username</label>
                <input
                  type="text"
                  value={svnUsername}
              ...
              <div>
                <label className="block text-xs text-muted-foreground mb-1">SVN Password</label>
                <input
                  type="password"
                  value={svnPassword}
