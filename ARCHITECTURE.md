# AI-OmniCommander — Architecture Reference

> **本檔為架構參考文件，行為規則一律以 CLAUDE.md 為準。**
> 標注「Legacy」的章節屬於舊的 SDK/`claude -p` 直接派工路徑——**已禁用 SDK/claude -p 派工，僅供歷史參考**（現行執行一律走外部 Claude Code session + MCP）。
...
## Architecture Overview（Legacy——已禁用 SDK/claude -p 派工，僅供歷史參考）
...
#### `server/src/agent/` — Agent lifecycle（Legacy——已禁用 SDK/claude -p 派工，僅供歷史參考）
| File | Purpose |
|------|---------|
...
| File | Purpose |
|------|---------|
| `SvnSpecService.ts` | Fetches spec documents from SVN for tasks. Uses root code extraction (e.g., `OV0101` → `OV`) to find the correct SVN folder, then recursively searches for matching .docx/.pdf files. Caches downloads in `data/uploads/{projectId}/`. Also hosts `fetchFolderSpecsForTask` (folder-source counterpart, delegates to FolderSpecSource). |

> 本地資料夾規格來源（`specFolders`）的設定格式與 git pull 安全鐵律屬**行為規則**，見 CLAUDE.md「本地資料夾規格來源」一節。
